...
\begin{notation}
	 $M^c\coloneq \R\setminus M$ bezeichne das Komplement von $M\subseteq\R$, $\overline{M}$ den Abschluss.
\end{notation}
...
\begin{proof}
	...
	Für die andere Richtung definiert die Funktion $\mu(A)\coloneq \int_A g \diff \lambda$ ein Maß auf den Borelmengen~$\mathfrak{B}$.
...
\begin{lemma}\label{lemma-b-graph}
	...
	und die Balance $b$ auf $H$ sei gegeben durch $b_v\coloneq b_v(\theta)$ für $v\in V$ und $b_{\newv_e}\coloneq x_e^-(\theta) - x_e^+(\theta)$ für $e\in E$.
...
\begin{remark}\label{remark-s-t-flow}
	In einem Nash-Fluss ist der statische Fluss $x(\theta)$ mit $x_e(\theta)\coloneq x_e^+(\theta)=x_e^-(\theta)$ nach Lemma~\ref{lemma-balance-0} für alle $\theta\in\R$ ein statischer $s$-$t$-Fluss.
