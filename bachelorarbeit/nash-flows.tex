...
\begin{proof}
	...
	Da jede offene Menge $O\subseteq\R$ $\sigma$-kompakt ist, also eine Darstellung als abzählbare Vereinigung kompakter Mengen -- hier $O=\bigcup_{n\in\N}(\{ x \in\R \mid d(x, O^c) \geq 1/n \} \cap [-n, n] )$ -- besitzt, ist jede offene Menge nach~\cite[Kap. VIII, 1.2 Folgerungen (e)]{Elstrodt2011} innen regulär.
	Das heißt, es gilt
...
\end{lemma} 
\begin{proof}
	$(i)$: Um zu zeigen, dass die Summe über die Balanceeinträge verschwindet, erkenne man, dass der Anteil einer Kante $e\in E$ in $b(V)$ gerade $x_e^+(\theta) - x_e^-(\theta)$ ist.
	Damit gilt:
