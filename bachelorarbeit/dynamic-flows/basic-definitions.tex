...
\begin{definition}[Netzwerk]
	Ein \emph{Netzwerk} $(V, E, u, s, t, \tau)$ ist ein gerichteter, endlicher Graph $(V,E)$ mit Ursprung $s\in V$, also ein Graph in dem $s$ alle Knoten in $V$ erreicht.
	Dabei heißen $s$ die \emph{Quelle} und $t\in V$ die \emph{Senke} des Netzwerks.
...
\begin{definition}[statischer Fluss]
	...
	Existieren zwei Knoten $s,t\in V$, sodass
	$$ \sum_{e\in\delta^+(v)} f_e - \sum_{e\in\delta^-(v)} f_e$$ für alle Knoten $v\in V\setminus\{ s, t \}$ verschwindet, für $s$ nicht-negativ und für $t$ nicht-positiv ist, so ist $f$ ein \emph{statischer $s$-$t$-Fluss mit Wert} $v(f):=\sum_{e\in\delta^+(s)} f_e - \sum_{e\in\delta^-(s)}f_e$.
\end{definition}

Man bemerke, dass hier ein statischer Fluss keine Kapazitätsbedingung erfüllen muss.
...
Über\-schreiten die Güter die Kapazität, so bildet sich eine Warteschlange vor dem Band.
