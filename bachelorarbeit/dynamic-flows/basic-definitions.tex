...
Außerdem sind parallele Kanten zwischen Knoten stets erlaubt, obwohl häufig die vereinfachte Schreibweise $vw:=(v,w):=e$ mit $v=\tail(e)$ und $w=\head(e)$ benutzt wird.

Für eine Menge $X$ an Knoten bezeichne $\delta^+(X):= \{ e\in E \mid \tail(e) \in X \notni \head(e) \}$ die Menge der ausgehenden Kanten von~$X$ und analog bezeichne $\delta^-(X)$ die Menge der eingehenden Kanten in~$X$.
...
\[ 
	f(E'):= \sum_{e \in E'} f_e \text{~~~ und ~~~} b(V'):=\sum_{v\in V'} b_v.
\]

\begin{definition}[Dynamisches Netzwerk]
...
\begin{definition}[Statischer Fluss]
	...
	\[ b_v := f(\delta^+(v)) - f(\delta^-(v)) \text{~~~ für $v\in V$}. \]
	Man nennt $f$ auch einen \emph{$b$-Fluss}.
	Gibt es zwei Knoten $s$ und $t$, sodass $b_v$
	für alle Knoten $v\in V\setminus\{ s, t \}$ verschwindet, für $s$ nicht-negativ und für $t$ nicht-positiv ist, so ist $f$ ein \emph{statischer $s$-$t$-Fluss mit Wert} $v(f):=\sum_{e\in\delta^+(s)} f_e - \sum_{e\in\delta^-(s)}f_e$.
\end{definition}
