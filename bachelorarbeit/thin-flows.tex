...
\begin{theorem}
		Algorithmus~\ref{algorithm-computation-thin-flow-without-resetting} berechnet schmale Flüsse ohne Zurücksetzen in einer Laufzeit von \todo{$\bigO(xyz)$}.
\end{theorem}
\begin{proof}
	...
	Daraus kann man nach \todo{add ref} in \todo{xyz Zeit} einen dünnsten Schnitt gewinnen, wodurch eine Gesamtlaufzeit von $\bigO((\size{b} + \size{u})^2 n^3 m)$ entsteht.
\end{proof}
