...
\todo{	Stattet man ein $s$-Netzwerk mit \emph{zurücksetzenden Kanten $E_1\subseteq E$} aus, so heißt das Tupel $(V, E, u, b, s, E_1)$ ein \emph{$s$-Netzwerk mit Zurücksetzen auf $E_1$}.}
\end{definition}
...
\begin{remark}\label{remark-thin-flow}
	...
	Ohne diese Bedingung würde dies nämlich nicht gelten, wie man in Bemerkung~\ref{remark-thin-flow-part-2} erörtert wird.
\end{remark}
\todo{
...
Da wir außerdem schon gesehen haben, dass bei Eingabe einer rationalen \problemThinFlow\ Instanz eine rationale Lösung existiert, bleibt die Hoffnung erhalten, dass auch \probTFwR\ in \PPAD\ liegt.
