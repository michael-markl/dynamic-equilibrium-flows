...
\begin{lemma}\label{lemma-thin-flows-have-minimal-congestion}
...
\end{proposition}
\begin{proof}
	Nach Korollar~\ref{cor-thin-flows-max-node-label-equals-max-edge-label}  gelten $f_{e}/u_{e}=q$ für $e\in\delta^+(X)$ und $f_{e} = 0$ für $e\in\delta^-(X)$.
...
\begin{lemma}
	Seien ein schmaler $s$-Fluss $f$ auf dem Netzwerk $(V, E, u, b)$ sowie ein  dünnster Schnitt $X$ gegeben.
	Sei $(X, E', u', b')$ das durch $X$ induzierte Teilnetz.
	Dann ist die Einschränkung $f':=f_{\mid E'}$ von $f$ auf $E'$ auch ein schmaler $s$-Fluss auf dem von $X$ induzierten Teilgraph.
\end{lemma}
\begin{proof}
	Lemma~\ref{lemma-thin-flows-have-minimal-congestion} und Proposition~\ref{prop-restricted-minimal-flow-is-b-flow-on-induced-instance} implizieren bereits, dass $f(\delta^-_E(X)) = 0$ gilt und $f'$ ein $b'$-Fluss ist. 
	Für $b_s = 0$ ist die Aussage trivial, denn dann ist $f$ der Nullfluss.
	Ist $b_s > 0$, so liegt $s$ auch im Schnitt $X$.
	
	Man zeige, dass die Knotenauslastungen $l$ und $l'$ von $f$ und $f'$ auf allen Knoten $v\in X$ übereinstimmen:
	...
	Daraus folgt $l'_v \geq l_v$, also insbesondere $l'_v = l_v$, wodurch auch $f'$ ein schmaler Fluss ist.
\end{proof}
...
\begin{lemma}
	...
	Dann hat jeder $s$-$v$-Pfad $P$ Auslastung $q^*=l_v$ für alle $v\in V\setminus X$.
\end{lemma}
\begin{proof}
	Ist $b$ der Nullvektor, so ist $f$ der Nullfluss und $l_v = 0$ für alle $v\in V$.
	Sonst ist $s$ in $X$ enthalten und jeder $s$-$v$-Pfad enthält mindestens eine Kante, die $X$ verlässt und nach Korollar~\ref{cor-easy-characterization-sparsest-cut} Auslastung $q^*$ besitzt.
