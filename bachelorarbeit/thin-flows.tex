...
\begin{corollary}\label{prop-easy-characterization-sparsest-cut}
	Gibt es in einem Netzwerk $(V, E, u)$ einen $b$-Fluss $f$ minimaler Auslastung $q^*$, so ist ein Schnitt $X$ mit $\delta^+(X)\neq\emptyset$ genau dann ein dünnster Schnitt, wenn alle ausgehenden Kanten von $X$ bzgl. $f$ Auslastung $q^*$ besitzen und alle eingehenden Kanten bzgl. $f$ verschwinden.
\end{corollary}
\begin{proof}
	Theorem~\ref{thm-strong-duality-sparsest-cut-min-flow} besagt, dass $q^*$ auch die Auslastung eines dünnsten Schnittes ist.
	...
	\]
	Gleichheit genau dann, wenn $f(\delta^-(X))=0$ und $f_{e}/u_{e} = q^*$ für alle $e\in\delta^+(X)$ gelten.
\end{proof}
...
\begin{lemma}
	...
	Der Fluss $f$ ist ein $b$-Fluss minimaler Auslastung $q^*$ und für $E\neq\emptyset$ ist $X:=\{ v\in V \mid l_v < q^* \}$ ein dünnster Schnitt.
\end{lemma}
\begin{proof}
	...
	Nach Theorem~\ref{thm-strong-duality-sparsest-cut-min-flow} genügt es zu
	zeigen, dass die Auslastung von $X$ gerade $q^*$ ist.
