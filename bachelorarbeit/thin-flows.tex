...
\begin{algorithmic}[1]
	...
	\State $f_{\mid E[X]} \gets f'_{\mid E[X]}$
...
\todo{
	\begin{theorem}
		Schmale Flüsse ohne Zurücksetzen können in $\bigO(xyz)$ Zeit berechnet werden können.
	\end{theorem}
}
...
\begin{example}
	...
	\begin{figure}
		...
		\begin{subfigure}{\textwidth}
			...
			\path [-Latex] (0) edge[bend left] node[above] {$10/4$} (4);
			\path [-Latex] (1) edge node[right] {$0/2$} (2);
			\path [-Latex] (1) edge[snake,segment amplitude=.4mm,segment length=2mm,line after snake=2mm] node[above] {$10/2$} (3);
			\path [-Latex] (2) edge[snake,segment amplitude=.4mm,segment length=2mm,line after snake=2mm] node[left] {$10/2$} (3);
			\path [-Latex] (3) edge node[above] {$0/1$} (4);
			\path [-Latex] (3) edge node[above] {$8/2$} (5);
			\end{scope}
		...
		\begin{subfigure}{\textwidth}
			...
			\begin{scope}[-Latex]
			\path [-Latex] (0) edge[snake,segment amplitude=.4mm,segment length=2mm,line after snake=2mm] node[above] {$11/4$} (1);
		...
		\begin{subfigure}{\textwidth}
			...
			\begin{scope}[-Latex]
			\path [-Latex] (0) edge[snake,segment amplitude=.4mm,segment length=2mm,line after snake=2mm] node[above] {$10/4$} (4);
