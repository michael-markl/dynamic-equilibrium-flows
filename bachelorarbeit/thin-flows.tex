...
\begin{proof}
	...
	Falls $x_{vw}'=0$ gilt, ist $l_w'=\min_{\tilde{v}w\in \delta^-(w)} \rho_{\tilde{v}w}(l_{\tilde{v}}', x_{\tilde{v}w}') = 0$.
...
\begin{proof}[Beweis von Theorem~\ref{thm-existence-thin-flow}]
	...
	\]
	Dabei sei $l'\in\R_{\geq 0}^V$ die zu $x'$ gehörige Knotenbewertung aus Lemma~\ref{lemma-thin-flow-node-labels}.
	...
	Die zu $x$ bzw. $x^n$ gehörige Knotenbeschriftung aus Lemma~\ref{lemma-thin-flow-node-labels} sei gegeben durch $l$ bzw. $l^n$. 
	Angenommen, es gelte $l_w < \rho_{vw}(l_v, x_{vw})$.
	Da die Zuordnung $x'\mapsto l'$ aus Lemma~\ref{lemma-thin-flow-node-labels} stetig ist, existiert ein $N\in\N$ mit $l_w^n<\rho_{vw}(l_v^n, x_{vw}^n)$ für alle $n\geq N$.
...
\begin{lemma}
	...
	%mit positivem Fluss enthalten keine maximal ausgelasteten Kanten und enthält ein $v$-$w$-Pfad $P$ mit positivem Fluss eine maximal ausgelastete Kante, so enthält jeder $v$-$w$-Pfad eine solche.
\end{lemma}
\begin{proof}
	\newcommand{\VK}{\text{VK}}
	\newcommand{\RK}{\text{RK}}
	...
	Bezüglich $\tilde{f}$ haben dann sowohl Vorwärtskanten als auch Rückwärtskanten eine geringere Auslastung als $q^*$.
...
%	
%	Sei ein $v$-$w$-Pfad $P$ mit positivem Fluss gegeben, der eine Kante aus $E'$ benutzt.
%	Angenommen, es würde $v=w$ gelten, so könnte man $f$ entlang des Kreises $P$ um $\gamma:=-\min_{e\in P} f_e > 0$ augmentieren und einen $b$-Fluss $\tilde{f}$ erhalten.
%	Da durch die Augmentierung die Auslastung der Kanten nur kleiner werden kann, ist auch die Auslastung von $\tilde{f}$ minimal und damit $q^*$, sodass die Menge der durch $\tilde{f}$ maximal ausgelasteten Kanten eine echte Teilmenge von $E'$ ist, weil mindestens eine Kante aus $E'$ in $P$ liegt, welche nach Augmentierung nicht mehr Auslastung $q^*$ hat.
%	Dies steht im Widerspruch zur Inklusionsminimalität von $E'$.
%	
%	Nun nehme man an, es existiere ein nichtleerer $v$-$w$-Pfad $Q$, der keine Kante aus $E'$ benutzt.
%	Es sei $\gamma_Q:=\min_{e\in Q} q^*u_e - f_e$ die minimale Flussmenge, die man $f$ entlang des Pfads $Q$ zugeben müsste, sodass mindestens eine Kante in $Q$ die Auslastung $q^*$ hat.
%	Außerdem sei $\gamma_P:=\min_{e\in P} f_e$ die maximale Flussmenge, die man aus $f$ entlang des Pfades $P$ entfernen kann.
%	Augmentiert man den Fluss $f$ nun um $\gamma:=\max\{ \gamma_P, \gamma_Q/2 \}$, so erhält man wieder einen $b$-Fluss $\tilde{f}$ minimaler Auslastung, der auf $P$ und $Q$ keine maximal ausgelasteten Kanten besitzt.
%	Dies erzeugt wieder einen Widerspruch zur Inklusionsminimalität von $E'$.
\end{proof}
...
\begin{proof}
	...
	Sonst sei $f$ ein $b$-Fluss minimaler Auslastung mit möglichst wenig maximal ausgelasteten Kanten.
	Sei $e^*=vw$ ein solche maximal ausgelastete Kante.
	Sei $X_1$ die Menge aller Knoten, die $v$ entlang eines gerichteten Weges auf Kanten mit positivem Fluss erreichen, und sei $X_2$ die Menge aller Knoten, die man ausgehend von $X_1$ entlang eines gerichteten Weges auf Kanten mit nicht-maximaler Auslastung 
	\todo{Finish this proof}
\end{proof}

\section{Erweiterung dynamischer Nash-Flüsse}\label{sec-nash-flow-extension}
