...
\begin{definition}[Schmaler Fluss]\label{def-thin-flow}
	Seien ein $s$-Fluss $x'$ mit Wert $F$ in einem azyklischen Netzwerk $(V, E, u)$ mit Ursprung $s$ und Versorgungsrate $d$ sowie eine Teilmenge $E_1\subseteq E$ der Kanten gegeben.
	Der Fluss $x'$ heißt \emph{schmaler Fluss}, falls eine Knotenbewertung $l'\in\R^V_{\geq 0}$ existiert, die die folgenden Bedingungen erfüllt:
