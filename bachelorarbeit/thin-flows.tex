...
\begin{lemma}
	...
	\begin{enumerate}[label=(T\arabic*)]
		...
		\item\label{def-thin-flow-x-zero} $l_w \leq l_v$, \tabto{4cm} für $vw\in E \setminus E_1$ mit $f_{vw}=0$,
		\item\label{def-thin-flow-x-positive} $l_w = \max(l_v, f_{vw} / u_{vw} )$,  \tabto{4cm} für $vw\in E\setminus E_1$ mit $f_{vw} > 0$,
		\item\label{def-thin-flow-resetting-edge} $l_w = f_{vw} / u_{vw}$,  \tabto{4cm} für $vw\in E_1$,
...
\begin{proof}
	...
	Man zeige, dass $l$ die Bedingungen~\ref{def-thin-flow-source}-\ref{def-thin-flow-no-resetting-edge} erfüllt, und benutze dabei die Darstellung von $l$ aus Proposition~\ref{prop-congestion-labels-dijkstra}.
	Wegen $l_s = 0$ gilt bereits~\ref{def-thin-flow-source}.
	Für eine Kante $vw\in E\setminus E_1$ mit $f_{vw}=0$ gilt $l_w\leq \rho_{vw}(l_v, f_{vw}) = l_v$, wodurch~\ref{def-thin-flow-x-zero} folgt.
	Ist $vw\in E$ mit $f_{vw} > 0$, so existiert ein $s$-$w$-Pfad mit positivem Fluss, der die Kante $vw$ benutzt, sodass die Auslastung dieses Pfades $l_w=\rho_{vw}(l_v, f_{vw})$, da der $s$-$v$-Teilpfad die Auslastung $l_v$ besitzt.
	...
	Man verwende eine Induktion über die Distanz eines Knotens $w$ zu $s$ in $E$ bezüglich der Kantenzahl, um zu zeigen, dass $l_w$ die Knotenauslastung von  $f$ ist.
...
\begin{lemma}\label{lemma-thin-flow-node-labels}
	Für jeden $F$-wertigen $s$-Fluss $x'$ in einem azyklischen Netzwerk $(V, E, u)$ mit Ursprung $s$ und Versorgungsrate $d$ sowie $E_1\subseteq E$ existiert eine eindeutige Lösung $(l_v')_{v\in V}$ des Gleichungssystems
	$$l_w' = \begin{cases}
		F/d, & \text{falls $w=s$,}\\
...
\begin{lemma}\label{lemma-equivalent-thin-flow}
	Ein $F$-wertiger $s$-Fluss $x'$ ist genau dann ein schmaler Fluss mit Zurück\-setzen auf $E_1$, wenn $x_{vw}'= 0$ für alle $vw\in E$ mit $l_w' < \rho_{vw}(l_v', x_{vw}')$ gilt.
