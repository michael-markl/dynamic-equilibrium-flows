...
In der $k$-ten Runde soll die obere Schranke $x_r/y_r$ der kleinstmögliche Bruch der Farey-Folge der Ordnung $2^k$ sein, der $x \leq x_r/y_r$ erfüllt.
\todo{Sobald man bei $F_{2M}$ angelangt, kann man sich sicher sein, dass alle möglichen Brüche untersucht wurden.}
Insgesamt werden also maximal $\lceil \log_2(M) \rceil$ solcher Runden benötigt.
...
\begin{proof}
	...
	\begin{description}
 	...
 	Wie bereits argumentiert, werden $\bigO(\log M)$ Runden benötigt.
 	Wird in einer Runde nur Fall~1 oder Fall~2 betrachtet, so werden nur konstant viele Vergleiche und Operationen benötigt.
 	Jedoch benötigt eine einzelne Ausführung von Fall~3 aufgrund der binären Suche unter Umständen $\bigO(\log M)$ viele Vergleiche.
...
\begin{proof}
	...
	\end{align*}
	Schließlich gelangt man zu $x$, durch Addition von $y$ und $\lowx$.
\end{proof}