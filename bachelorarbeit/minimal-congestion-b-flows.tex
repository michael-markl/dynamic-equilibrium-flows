...
\begin{definition}[Farey-Folge]
	Für ein $n> 1$ sei $F_n$ die aufsteigend sortierte Folge aller irreduziblen, d.h. vollständig gekürzten, Brüche in $[0,1]$, deren Zähler bzw. Nenner ganze Zahlen von $0$ bzw. $1$ bis $n$ sind.
	Man nennt $F_n$ die \emph{Farey-Folge der Ordnung $n$} und $x_i^n/y_i^n$ bezeichnet den $i$-ten Eintrag von $F_n$.
\end{definition}
...
\begin{lemma}[Lemma von Bézout]\label{lemma-von-bezout}
	Für zwei ganze Zahlen $a, b\in\Z$ existieren zwei weitere ganze Zahlen $s,t\in\Z$, sodass $\ggT(a,b) = sa + td$ gilt.
\end{lemma}
...
\begin{proof}
	...
	Da ein Nachfolger existieren muss, kann man $h<k$ voraussetzen.
