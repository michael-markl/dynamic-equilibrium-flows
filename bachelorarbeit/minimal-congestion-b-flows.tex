...
\begin{definition}[Schnitt]
	In einem gerichteten Netzwerk $(V, E, u)$ wird eine Teilmenge $X\subseteq V$ auch \emph{Schnitt} genannt.
	Sind Knotenbalancen $b\in\R^V$ gegeben und ist $\delta^+(X)$ nichtleer, so bezeichne $b(X) / u(\delta^+(X))$ die \emph{Auslastung des Schnittes $X$}.
...
\begin{definition}[Residualgraph eines $b$-Flusses]
	...
	Der \emph{Residualgraph von $G$ bezüglich $f$} sei definiert durch $G_f \coloneq (V, E_f)$ mit \[
...
\begin{theorem}\label{thm-strong-duality-sparsest-cut-min-flow}
	Existiert in einem gerichteten Netzwerk $(V, E, u)$ mit Balancen $b\in\R^V$ ein $b$-Fluss und ist $E$ nichtleer, so ist die minimale Auslastung eines $b$-Flusses gerade die Auslastung eines dünnsten Schnittes; das bedeutet
	\[
...
Verwendet man beispielsweise den wohlbekannten Push-Relabel-Algorithmus von Goldberg und Tarjan, \todo{add cite} so kann ein maximaler $s$-$t$-Fluss mit $\bigO(n^2 m)$ Operationen berechnet werden, wobei $n$ die Anzahl der Knoten und $m$ die Anzahl der Kanten des Graphen ist.
