...
\begin{definition}[Augmentieren eines Flusses]
	\newcommand{\VK}{\text{VK}}
	\newcommand{\RK}{\text{RK}}
	Seien $f$ ein $b$-Fluss und $C$ ein Kreis in seinem Residualgraph $G_f$.
	Man setze \[
	\gamma_{\max} := \min\left\{ \frac{f_e}{k_{\overleftsmallarrow{e}} - k_{\overrightsmallarrow{e}}} ~\middle|~ e\in E(C), k_{\overleftsmallarrow{e}} > k_{\overrightsmallarrow{e}} \right\},
	\]
	wobei $k_d$ die Anzahl der Vorkommen der Kante~$d$ in $C$ sei.
	Für ein $\gamma\in [0, \gamma_{\max}]$ bezeichne~$g$, gegeben durch
	\[
		g_e := f_e + (k_{\overrightsmallarrow{e}} - k_{\overleftsmallarrow{e}}) \gamma \geq 0,
	\]
...
\todo{Kanten parallel ok, aber oft als $vw$ bezeichnet}
...
Mit diesem Lemma kann nun ein hinreichendes und notwendiges Optimalitätskriterium formuliert werden:
