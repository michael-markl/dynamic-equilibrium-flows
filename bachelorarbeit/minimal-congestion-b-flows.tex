...
\begin{lemma}
	...
	\[ \left\{ \frac{p}{q} ~\middle|~ p\in\Z, q\in\N, \abs{p} \leq k2^{\size{A} + \size{b} + \size{c^*} - m^2}, 1\leq q \leq 2^{\size{A}-m^2} \right\}. \]
\end{lemma}
\begin{proof}
	...
	\]
	Der Zähler wird mit $\size{A_B^i} \leq \size{A}+\size{b}$ nach Proposition~\ref{prop-rational-bounds-encoding} wie folgt abgeschätzt:
	\[
	\abs{ \sum_{i\in B} c_i \det(A_B^i) } \leq \sum_{i\in B} (2^{\size{c_i} - 1} - 1) 2^{\size{A} + \size{b} - m^2} \leq k 2^{\size{c^*} + \size{A} + \size{b} - m^2}
	\]
	Der Nenner ist betraglich durch $2^{\size{A} - m^2}$ beschränkt, sodass die Aussage folgt.
\end{proof}
