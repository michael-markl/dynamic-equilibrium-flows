...
\begin{example}\label{example-min-con-flow}
	...
	Die maximale Kantenauslastung durch $x$ ist $4$, welche auf den Kanten $(b,a), (f,b),(f,c)$ und $(h,f)$ angenommen wird.
...
\begin{center}
	\begin{tikzpicture}
	\node[draw, circle, outer sep=4pt] (z1) at (2,0) {$Z_1$};
	\node[draw, circle, outer sep=4pt] (z2) at (4,0) {$Z_2$};
	\node[draw, circle, outer sep=4pt] (z3) at (0,0) {$Z_3$};
...
Insbesondere folgt also, dass jeder dünnste Schnitt eines auslastungsminimalen Flusses alle Knoten enthält, die er im Residualgraphen erreichen kann.
...
Die erste Bedingung ist zusammen mit der Nichtnegativität von $f_e/u_e$ gerade die $b$-Fluss-Eigenschaft.
