...
\begin{definition}[Auslastungsminimaler $b$-Fluss]
	...
	Dann heißt $f_e/u_e$ die \emph{Auslastung der Kante $e$ durch $f$} und die maximale Kantenauslastung $c(f)\coloneq \max_{e\in E} f_e/u_e$ bezeichnet die \emph{Auslastung des Flusses $f$}.
...
\begin{definition}[Schnitt]
	...
	Sind zusätzlich Knotenbalancen $b\in\R^V$ mit $b(V) = 0$ gegeben und ist $\delta^+(X)$ nichtleer, so bezeichne $b(X) / u(\delta^+(X))$ die \emph{Auslastung des Schnittes $X$}.
...
\begin{theorem}[Schwache Dualität]\label{thm-weak-duality-min-con-flow}
 	In einem gerichteten Netzwerk $(V, E, u)$ mit Balancevektor $b\in\R^V$ ist die Auslastung eines $b$-Flusses $f$ mindestens so groß wie die Auslastung eines Schnittes $X$ mit ausgehenden Kanten; das heißt \[\max_{e\in E} \frac{f_e}{u_e} \geq \frac{b(X)}{u(\delta^+(X))}.\]
\end{theorem}
\begin{proof}
...
\begin{example}\label{example-min-con-flow}
	...
	In der Form von Theorem~\ref{thm-weak-duality-min-con-flow} würde man argumentieren mit $X\coloneq V\setminus Y$, für das $b(X) = 4$ und $u(\delta^+(X)) = 1$ gilt.
...
\begin{definition}[Doppelgraph]
	Der Doppelgraph $G^\leftrightarrow$ eines Graphen $G=(V,E)$ ist das Paar $(V, \overrightsmallarrow{E}\cup \overleftsmallarrow{E})$, wobei $\overrightsmallarrow{E}\coloneq \{ \overrightsmallarrow{e} \mid e\in E \}$ die Menge der Vorwärtskanten, die die Richtung der Ursprungskante beibehalten, und $\overleftsmallarrow{E}\coloneq \{ \overleftsmallarrow{e} \mid e\in E \}$ die Menge der Rückswärtskanten, die die Richtung der Ursprungskante umkehren, sind.
\end{definition}
\begin{definition}[Residualgraph eines $b$-Flusses]
	...
	Der \emph{Residualgraph von $G$ bezüglich $f$} ist definiert durch $G_f \coloneq (V, E_f)$ mit \[
...
\begin{proof}[Beweis von Lemma~\ref{lemma-min-flow-criterion}]
	...
	Die Strömung $g\coloneq f'\Delta f$ besitzt nach Proposition~\ref{prop-difference-b-flows-stream} eine Dekomposition in Zyklen $g = \lambda_1 \cdot C_1 +\dots + \lambda_k \cdot C_k$ mit $\lambda_i > 0$ für $i\in[k]$.
...
\begin{definition}[Zusammenhangsgraph]
	...
	\]
	Dann nennt man den azyklischen Graphen $G^\sim\coloneq (V^\sim, E^\sim)$ den \emph{Zusammenhangsgraphen von $G$}, wobei die Knoten $V^\sim$ die Menge der Äquivalenzklassen -- also die Menge der \emph{(starken) Zusammenhangskomponenten} -- sind und eine Kante $xy$ in $E^\sim$ genau dann existiert, wenn $x\neq y$ gilt und es eine Kante $vw$ in $E$ gibt, sodass $v$ in der Zusammenhangskomponente $x$ und $w$ in der Zusammenhangskomponente $y$ liegen.
\end{definition}
...
\begin{proof}
	...
	Man setze $\gamma_\VK \coloneq \min_{\overrightsmallarrow{e}\in C_\VK} q^*u_e - f_e > 0$ als die minimale Flussmenge, die man jeder Kante in $C_\VK$ zufügen müsste, sodass mindestens eine Kante darin mindestens Auslastung $q^*$ erhält.
	Weiter sei $\gamma_\RK \coloneq \min_{\overleftsmallarrow{e}\in C_\RK} f_e > 0$ die minimale Flussmenge der Rückwärtskanten in $C$.
...
Mit diesem Lemma kann nun ein hinreichendes und notwendiges Optimalitäts\-kriterium formuliert werden:
...
Ein $b$-Fluss ist kostenminimal genau dann, wenn der zugehörige, so definierte Residualgraph keine negativen Zyklen hat, also Zyklen $C$ mit $c(E(C)) < 0$ (\todo{Add cite}).
...
\begin{definition}[Ecke konvexer Menge]
	Für eine konvexe Menge $P$ heißt $x\in P$ eine \emph{Ecke von $P$}, falls $x$ das einzige Element des Schnittes von $P$ und einer Stützhyperebene $H\coloneq \{ x\in\R^n \mid d^T x = \delta \}$ mit $d\in\R^n$ und $\delta\in\R$ ist.
\end{definition}

\begin{definition}[Zulässige Basislösung]
Für ein lineares Programm in Standardform mit $P\coloneq P^=(A, b)$ mit $\rank(A) = m$ heißt eine $m$-wertige Teilmenge $B$ von $[n]$ Basis und $N\coloneq [n]\setminus B$ die zugehörige Nichtbasis zu $B$, falls $A_B$ invertierbar ist.
...
\begin{proof}
	...
	Ist andersherum $f$ ein $b$-Fluss mit Auslastung $q^*\leq p/q$, so ist der Fluss $\tilde{f}$, der $f$ um $\tilde{f}_{(s,v)}\coloneq b_v$ für $b_v > 0$ und $\tilde{f}_{(v,t)}\coloneq -b_v$ für $b_v < 0$ augmentiert, ein maximaler $s$-$t$-Fluss, der die Kapazitäten $\nu$ einhält.
