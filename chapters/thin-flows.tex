...
\begin{definition}[Schmaler Fluss mit Zurücksetzen]\label{def-thin-flow}
	...
	\item $l_s' = F/d$,
	\item $l_w' \leq l_v'$, \tabto{4cm} für $vw\in E \setminus E_1$ mit $x'_{vw}=0$,
	\item $l_w' = \max(l_v', x'_{vw} / u_{vw} )$,  \tabto{4cm} für $vw\in E\setminus E_1$ mit $x'_{vw} > 0$,
	\item $l_w' = x'_{vw} / u_{vw}$,  \tabto{4cm} für $vw\in E_1$.
	\end{enumerate}
\end{definition}
...
\begin{proof}
	...
	\begin{description}[leftmargin=0cm, topsep=0cm, itemindent=\parindent]
		...
		Ist $vw\in E_1$, so ist also Bedingung (4) erfüllt.
		Für Bedingung (2) setze man $g_{vw}=x_{vw}'(\theta)=0$ voraus.
		...
		Teilt man wieder durch $\varepsilon$, so erhält man für $\varepsilon  \rightarrow 0$ Bedingung (3) mit $l_w'(\theta) \leq l_v'(\theta)$ und dem Resultat des letzten Absatzes.
		...
		Bedingung~(2) wird erfüllt, indem man durch $\theta_\varepsilon'-\theta$ teil und $l_w'(\theta) = l_v'(\theta)$ für $\varepsilon\rightarrow0$ erhält.
		
		Für Bedingung~(3) bleibt zu zeigen, dass $x_{vw}'(\theta) /u_{vw}\leq l_w'(\theta)$ gilt.
