...
\begin{proof}
	...
	Um Aussage (iii) zu zeigen, genügt es zu beweisen, dass $\tilde{l}_v(\theta) = l_v(T) + (\theta - T) l_v'$ für $\theta\in [T, T+\alpha]$, da für $\theta < T$ und $w\neq s$ ist $\tilde{l}_w(\theta) = \min_{vw\in\delta^-(w)} \tilde{T}_{vw}(\tilde{l}_v(\theta))$
