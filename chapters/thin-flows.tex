...
\begin{proof}
	...
	Um Aussage (iii) zu zeigen, zeige man zunächst $T_{vw}(\theta) = \tilde{T}_{vw}(\theta)$  für $\theta \leq l_v(T)$:
	Dies gilt genau dann, wenn $\tilde{q}_{vw}(\theta) = q_{vw}(\theta)$.
	Ist $x_{vw}' = 0$, so stimmt der Zu- bzw. Abfluss von $f$ und $\tilde{f}$ an $vw$ und damit auch die Wartezeiten zur Zeit $\theta$ überein.
	Ist $x_{vw}'$ positiv, so gilt $\theta + \tau_{vw} \leq l_v(T)+\tau_{vw} \leq l_w(T)$ nach (i).
	Demnach ist $\tilde{F}_{vw}^-(\theta + \tau_{vw}) = F_{vw}^-(\theta + \tau_{vw})$ und, da auch $\tilde{F}_{vw}^+(\theta)$ mit  $F_{vw}^+(\theta)$ übereinstimmt, sind die Wartezeiten identisch. 
	
	Nun sieht man leicht, dass $(l_v(\theta))_{v\in V}$ das Gleichungssystem in Proposition~\ref{prop-arrival-times-vector} für $\theta \leq T$ erfüllt.
	Für $\theta \in (T, T+\alpha]$ gilt:
	
%	Für einen bzgl. $\tilde{f}$ zum Zeitpunkt $\theta\leq l_w(T)$ kürzesten $s$-$w$-Pfad $P$, der ausschließlich aktive Kanten benutzt, gilt
%	$\tilde{l}_w(\theta) = \tilde{l}^P(\theta)$.
%	Induktiv zeige man $\tilde{l}_v(\theta) = l_v(\theta)$ für alle Knoten $v$ in $P$.
%	Für $s$ gilt dies offenbar und für eine Kante $uv$ in $P$, für die nach Induktionsannahme $\tilde{l}_u(\theta) = l_u(\theta)$ gilt, ist $\tilde{l}_v(\theta) = \tilde{T}_{uv}(\tilde{l}_u(\theta)) = \tilde{T}_{uv}(l_u(\theta)) = T_{uv}(l_u(\theta))$.
	
%	genügt es zu beweisen, dass $(l_v(T) + (\theta - T) l_v')_{v\in V}$ das Gleichungssystem in Proposition~\ref{prop-arrival-times-vector} für $\theta\in (T, T+\alpha)$ erfüllt,
%	da für $\theta < T$ und $w\neq s$ ist $\tilde{l}_w(\theta) = \min_{vw\in\delta^-(w)} \tilde{T}_{vw}(\tilde{l}_v(\theta)) = \min$
%	
%	Ist $vw\in E$ und $T\in\Theta_{vw}$ mit 
\end{proof}