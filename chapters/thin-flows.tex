...
\begin{proof}
	...
	\todo{don't argue with 0} Für Aussage (iii) zeige man $T_{vw}(l_v(\theta)) \leq l_w(\theta)$.
...
\begin{proof}
	...
	Für zur Zeit $T$ aktive Kanten $vw$ folgt daher $l_w'\leq l_v'$ nach~\ref{def-thin-flow-x-zero}, wodurch man $\tilde{T}(l_v(T) + (\theta - T)l_v')\geq l_v(T) + (\theta - T)l_v' + \tau_{vw}\geq l_v(T) + (\theta - T)l_w' + \tau_{vw} = l_w(T) + (\theta-T)l_w'$ erhält.
