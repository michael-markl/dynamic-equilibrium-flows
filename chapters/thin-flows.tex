...
\begin{definition}[$\alpha$-Erweiterung]
	Seien ein dynamischer Nash-Fluss $f$ mit Zeithorizont $T$ und ein $\tilde{d}$-wertiger schmaler Fluss mit Zurücksetzen auf $E_1 := \{ vw\in E \mid q_{vw}(l_v(T)) > 0 \} $ im Graphen $G_T$ bei Versorgungsrate $\tilde{d}$ und ein $\alpha > 0$ gegeben.
...
\begin{figure}[h!]
	...
	\caption{In der Abbildung ist ein Netzwerk zu erkennen mit Kantenbeschriftung $\tau_e / u_e$.
		Man betrachte den dynamischen Nullfluss mit Zeithorizont $0$. Dann ist $x'$ mit $x_{sv}'=x_{vt}'=0$ und $x_{st}'=2$ nach \cite[Definition 6]{Koch2011} ein schmaler Fluss mit Zurücksetzen auf $\emptyset$ mit zugehörigen Knotenbewertungen $l_s' = 1$, $l_v' = 0$, $l_t' = 2$.
		Aber für eine $\alpha$-Erweiterung ist $l_v(\theta) = \theta + 1 > \theta = l_v(0) + l_v' \theta$.} 
	\label{figure-labels}
\end{figure}