...
Die Voraussetzung, dass hierbei die entsprechenden Ableitungen existieren, kann man dadurch rechtfertigen, dass absolut stetige Funktionen fast überall differenzierbar sind (\cite[4.12 Folgerung~b)]{Elstrodt2011}).
Zudem gilt nach \cite[Aufgabe 4.10]{Elstrodt2011} die Substitutionsregel für absolut stetige Funktionen, wodurch $\int_{l_v(\theta_1)}^{l_v(\theta_2)} f_{vw}^+(t) \diff t = \int_{\theta_1}^{\theta_2} f_{vw}^+(l_v(t)) l_v'(t) \diff t$ gefolgert werden kann.
...
Es fehlt noch Bedingung~\ref{def-thin-flow-no-resetting-edge} zu zeigen.
...
\begin{definition}
	Seien ein dynamischer Nash-Fluss $f$ mit Zeithorizont $T$ und ein schmaler Fluss mit Zurücksetzen auf $E_1 := \{ vw\in E \mid q_{vw}(l_v(\theta)) > 0 \} $ im Graphen $G_T$ zum Zeitpunkt $T$ und ein $\alpha > 0$ gegeben.
	
	Man erhalte die \emph{$\alpha$-Erweiterung $\tilde{f}$ von $f$}, indem man die Werte aus $f$ ergänze und für alle zur Zeit $T$ aktiven Kanten $vw\in E_T$ den Zu- bzw. Abfluss setze auf
