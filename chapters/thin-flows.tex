...
\end{lemma}
\begin{proof}\todo{don't argue with 0}
...
\begin{definition}
	...
	Man erhalte die \emph{$\alpha$-Erweiterung $\tilde{f}$ von $f$} aus $f$, wobei man für alle $vw\in E$ den Zu- bzw. Abfluss setze auf
...
\begin{proof}
	...
	Für \ref{def-feasible-flow-no-negative-flow} zeige man $\tilde{F}^+_{vw}(\theta)\geq \tilde{F}_{vw}^-(\theta+\tau_{vw})$ für alle $vw\in E, \theta\in\R$.
	...
	Für $x_{vw}'>0$ und $\theta\in(T, T+\alpha l_v')$ 
	
	
	
	 bemerke man zunächst, dass für $vw\in E$ mit $x_{vw}'=0$ die Kapazitätsbedingung \ref{def-feasible-flow-capacity} und \ref{def-feasible-flow-no-flow-at-node} erfüllt sind.
