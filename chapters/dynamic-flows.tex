...
\begin{definition}[Zulässiger dynamischer Fluss]
	...
	\begin{enumerate}[label=(F\arabic*)]
		...
		\item\label{def-feasible-flow-no-flow-at-node} Bis auf Quelle und Senke erfüllt jeder Knoten $v$ Flusserhaltung,\\ d.h. $\forall\theta\in\R: \sum_{e\in\delta^-(v)}f^-_e(\theta) - \sum_{e\in\delta^+(v)} f_e^+(\theta) = 0$.\\
