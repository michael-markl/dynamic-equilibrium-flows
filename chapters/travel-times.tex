...
\begin{proof}
	...
	Da jeder Knoten $w\neq s$ eine eingehende Kante besitzt, ist jeder Knoten auch von $s$ aus in $G'$ erreichbar.
	Für einen $s$-$w$-Pfad $P$ in $G'$ gilt dann $\tilde{l}_w = T^P(\theta) \geq l_w(\theta)$.
	Außerdem gilt für einen $s$-$w$-Pfad $P$ mit ausschließlich aktiven Kanten $\tilde{l}_w \leq T^P(\tilde{l}_s) = T^P(\theta) = l_w(\theta)$.
