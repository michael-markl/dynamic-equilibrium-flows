...
\begin{proof}
	...
	Der durch solche Kanten $uv\in E$, welche unter allen eingehenden Kanten von $v$ die Zeit $T_{uv}(\tilde{l_u})$ minimiert, induzierte Teilgraph ist azyklisch,
	...
	Folgt man ausgehend von einem Knoten $w\neq s$ solange wie möglich eingehenden Kanten $uv$ in diesem Teilgraphen, so erhält man also einen $s$-$w$-Pfad $P$, für den $\tilde{l}_w = T^P(\theta) \geq l_w(\theta)$ gilt.
