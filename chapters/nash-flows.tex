...
Dabei betrachte man folgende Intuition:
 Partikel, die zur Zeit $\theta\in\R$ bei $s$ starten und sich auf einem kürzestem Weg zu $t$ bewegen  -- also zur Zeit $l_t(\theta)$ in $t$ ankommen --, überholen andere Partikel, falls $b_s(\theta) > -b_t(\theta)$.
...
\end{lemma}
\begin{proof}
	Zu Aussage (i): Für $\theta'\in(\theta-\varepsilon, \theta)$ gilt $F_{vw}^+(l_v(\theta')) < F_{vw}^+(l_v(\theta))$.
	...
	Nach Lemma~\ref{lemma-only-active-edges} ist $\xi\in l_v(\Theta_{vw})$ für fast alle $\xi\in Q_\varepsilon$; insbesondere existiert ein $\theta_\varepsilon\in (\theta-\varepsilon, \theta)$ mit $\theta_\varepsilon\in\Theta_{vw}$.
