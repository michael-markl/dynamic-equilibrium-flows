...
\begin{definition}
	Für eine Kante $vw\in E$ bezeichne $x_{vw}^+(\theta):= F_{vw}^+(l_v(\theta))$ den Fluss, der die Kante $vw$ betreten hat, bevor Partikel, die zur Zeit $\theta$ in $s$ starten, $v$ erreichen können.
	
	Mit $x_{vw}^-(\theta):= F^-_{vw}(l_w(\theta))$ bezeichne man den Fluss, der die Kante $vw$ verlassen hat, bevor Partikel, die zur Zeit $\theta$ in $s$ starten, $w$ erreichen können.
...
Die Ergebnisse aus Lemma~\ref{lemma-only-active-edges} und Lemma~\ref{lemma-b-graph} werden im folgenden Theorem gesammelt:
