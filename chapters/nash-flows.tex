...
Für eine äquivalente Umschreibung dieser Definition, benötigen wir folgendes Lemma der Mengentheorie:
...
Partikel, die in $\theta\in\R$ bei $s$ starten und sich auf einem kürzestem Weg zu $t$ bewegen, überholen andere Partikel, falls $b_s(\theta) > -b_t(\theta)$.
Falls jedoch $b_s(\theta) < - b_t(\theta)$ gilt, wurde das Partikel bereits von anderen überholt.
Ein Nash-Gleichgewicht sollte diese Eigenschaft also erfüllen.
...
Die Ergebnisse aus Lemma~\ref{lemma-only-active-edges} und Lemma~\ref{lemma-b-graph} werden im folgenden Theorem gesammelt, welches Nash-Gleichgewichte in dynamischen Flüssen charakterisieren soll:
...
In diesem Abschnitt werden einige Ergebnisse über Nash-Flüsse gesammelt, die in Abschnitt~\ref{sec-thin-flows} gebraucht werden.
...
\begin{lemma}\label{lemma-nash-flow-waiting-queue-implies-active-edge}
	...
	\end{enumerate}
	Insbesondere verschwindet für $\theta\in\overline{\Theta_{vw}^c}$ die Wartezeit $q_{vw}(l_v(\theta))$  nach (iii).
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{lemma-x-locally-constant}
	In einem dynamischen Nash-Fluss ist $x_e$ eingeschränkt auf $\bar{\Theta_e^c}$, also dem Abschluss der Menge der inaktiven Zeitpunkten, für jede Kante $e\in E$ lokal konstant.
\end{lemma}
\begin{proof}
