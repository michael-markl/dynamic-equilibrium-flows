...
\end{proposition}
\begin{proof}
	\todo{Das funktioniert hier noch gar; vlt mal mit x' probieren und dann integrieren}
	...
	Also existiert eine streng monoton wachsende Folge $(\xi_n)_{n\in\N_0}$ reeller Zahlen mit $\xi_0 = 0$ und $\lim_{n\to\infty} \xi_n = \infty$ und der Eigenschaft, dass $E_\theta$ für alle $\theta\in(\xi_n, \xi_{n+1})$ identisch ist.
	Außerdem kann sich die Aktivität von Kanten im Intervall $(-\infty, 0)$ nicht verändern, womit $E_\theta$ auch auf $(-\infty, 0)$ konstant ist.
	
	Für $\theta \leq 0$ ist $x(\theta)$ der Nullfluss, da die Funktionen $f_e^+$ und $f_e^-$ 
\end{proof}