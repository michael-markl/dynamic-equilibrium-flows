...
\begin{proof}
	...
	Also ist der $s$-$t$-Fluss $x(\xi) - x(\theta-\varepsilon)$ für $\xi\in [\theta - \varepsilon, \theta + \varepsilon]$ azyklisch und besitzt eine $s$-$t$-Wegezerlegung.
	Addiert man diese zur $s$-$t$-Wegezerlegung von $x(\theta-\varepsilon)$, erhält man eine $s$-$t$-Wegezerlegung von $x(\xi)$, was ein Widerspruch zur Annahme ist.
\end{proof}