...
\begin{proof}
	...
	Dann sind auch alle $b_{\newv_e} = 0$ und nach Lemma~\ref{lemma-balance-0} gilt $b_s(\theta) + b_t(\theta) = 0$, da $g$ statischer $b$-Fluss ist.
	Das bedeutet $f$ fließt ohne Überholungen.
	
	Angenommen, $f$ fließe ohne Überholungen.
	Dann gilt $\sum_{e\in E} b_{\newv_e} = 0$ unter Verwendung von Lemma~\ref{lemma-balance-0} für alle $\theta\in\R$.
