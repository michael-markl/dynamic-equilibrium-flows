...
\todo{$l_v$ ist absolut stetig}

\begin{remark}
	Diese Definition weicht ab von Koch und Skutella und entspricht derjenigen aus~\cite[Definition 1]{Cominetti2015}:
	...
	Daher wird die Bedingung in Definition~\ref{def-flow-along-active-edges} bereits impliziert.
	In~\cite[Example 2]{Cominetti2015} wird außerdem ein Beispielfluss angegeben, der zeigt, dass die Forderung sogar echt stärker ist.
\end{remark}
...
\begin{proof}
	...
	Sei $\omega:=\sup\{ \omega\in\R \mid l_v(\theta) \geq T_{uv}(l_u(\omega)) \}$ der späteste Startzeitpunkt in $s$, sodass man unter Benutzung von $uv$ spätestens zum Zeitpunkt $l_v(\theta)$ zu $v$ gelangt.
	Man beachte, dass $\omega=-\infty$ gilt, falls solch ein Startzeitpunkt nicht existiert.
	Es ist $\omega \leq \theta$, da $T_{uv}\circ l_u$ monoton wachsend ist und für $\omega > \theta$ wäre $T_{uv}(l_u(\omega)) > l_v(\theta)$.
	Wegen der Monotonie von $l_v$ und nach Definition von $\omega$ gilt $l_v(\theta')\leq l_v(\theta)< T_{uv}(l_u(\theta'))$ für $\theta'\in (\omega, \theta]$;
	insbesondere ist die Kante $uv$ im Intervall $(\omega, \theta]$ nicht aktiv und nach Voraussetzung gilt $f_{uv}^+(l_u(\theta')) = 0$ für fast alle $\theta'\in (\omega, \theta]$.
	Daher gilt $F_{uv}^+(l_u(\theta)) = F_{uv}^+(l_u(\omega))$. Nach Proposition~\ref{prop-feasible-flow}~(\ref{prop-feasible-flow-det-outflow}) ist dann $F_{uv}^-(T_{uv}(l_u(\theta))) = F_{uv}^-(T_{uv}(l_u(\omega))\leq F_{uv}^-(l_v(\theta))$ wegen Monotonie von $F_{uv}^-$ und der Definition von $\omega$.
	...
	\todo{Falsch:} Also ist $f_{uv}^+$ in einer Umgebung um $l_u(\theta)$ fast überall gleich $0$.
