...
\begin{definition}
	...
	Der Fluss $f$ heißt \emph{zulässiger $b$-Fluss}, falls er keine Kapazität überschreitet, d.h. $\forall e\in E: f_e \leq u_e$, und Flusserhaltung bzgl. $b$ gewährt, d.h. $\forall v\in V: \sum_{e\in\delta^+(v)}f_e - \sum_{e\in\delta^-(v)}f_e = b_v$.
\end{definition}
...
\begin{lemma}
	...
	\begin{enumerate}[(i)]
		\item Für alle Kanten $e\in E$ von G und zu jedem Zeitpunkt $\theta\in\R$ ist $x_e^+(\theta) = x_e^-(\theta)$.
		\item Für alle $\theta\in\R$ ist $g^\theta$ ein statischer $b^\theta$-Fluss.
	\end{enumerate}
\end{lemma}
\begin{proof}
	$(i)\Rightarrow(ii)$: Sei $\theta\in\R$ gegeben.
		Nach Voraussetzung ist $b^\theta_{\newv_e} = 0$ für alle $e\in E$ und eine Kante $uv$ hat in $\sum_{v\in V} b_v^\theta$ nur bei den Summanden $b_u^\theta$ und $b_v^\theta$ einen Beitrag, die sich beide gegenseitig aufheben, und man erhält
	...
	\sum_{e\in\delta_H^+(v)} g_e^\theta - \sum_{e\in\delta^-_H(v)}g_e^\theta
		...
		\todo{Jetzt hab ich noch nicht 'zulässig' gebraucht}
\end{proof}